...
object Importer {
  ...
  implicit class RichString(in: String) {
    ...
    def formatPercent: String = percentFormat.format(BigDecimal(in))
    ...
    def formatDate: String = in.takeWhile(_ != ' ')
  ...
  def csvFile[A](
    ...
    // you will likely need to convert the broken MSDOS encoding on the exported files - 
    // iconv -f iso-8859-1 -t UTF-8//TRANSLIT filein -o fileout

    val reader = CSVReader.open("calc/src/test/resources/testing-tables/extracted3/" ++ file ++ ".csv")
  ...
  def main(args: Array[String]): Unit = {
    ...
    allTimePeriods.map{ on =>
      ...
      // the formatted time period, possibly shortened back into a year again. 
      val onP = TaxYear.unapply(on) match {
        case Some(taxYear) => taxYear.toString
        case None => on.toString
      }

      val c2 = class2.get(on) match {
        case None => ""
        case Some(c) =>
          ...
          s"""|class-two {
              |  weekly-rate: ${c("Men Wk Rte").formatMoney}
              |  small-earnings-exception: ${c("SEE Limit").formatMoney}
              |  $finalDate 
              |  $hrpDate
              |  no-of-weeks: ${c("No of Wks")}
              |  qualifying-rate: ${c("EF Qual Year").formatMoney}
              |}""".stripMargin.indent(1)
      }

      val c3 = class3.get(on) match {
        case None => ""
        case Some(c) =>
          ...
          s"""|class-three {
              |  week-rate: ${c("Wk Rte").formatMoney}
              |  $finalDate 
              |  no-of-weeks: ${c("No Of Weeks")}
              |  lel: ${c("LEL").formatMoney}
              |  qualifying-rate: ${c("EF Qual Year").formatMoney}
              |}""".stripMargin.indent(1)
          
      }

      val c4 = class4.get(on) match {
        case None => ""
        case Some(c) =>
